...
// string. Despite its name (kept for historical reasons), this helper is
// not about Accept negotiation — it is used to derive the bare type that
// keys the producer/consumer maps registered on a [RoutableAPI]. Those
// maps are looked up by the bare media type, so an entry registered as
